...
// The only edit in this file, and the entire point of the fork: `opus` 0.3.1 verbatim,
// with the -sys crate underneath it swapped for one that links a prebuilt static
// archive instead of building opus with cmake. Keeping the rest byte-identical to
// upstream is deliberate — it means a future `opus` release can be re-forked by copying
// its lib.rs and changing this line again, and it means the safe-wrapper semantics
// (which are what the consuming projects are actually written against) are not
// something this repository has any opinion about.
extern crate libopus_prebuilt_sys as ffi;
