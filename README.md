...
- the archive is **reproducible**, and CI asserts it: a second runner rebuilds the same
  target and the two `libopus.a` must hash identically, against each other and against the
  `sha256(library)` the first one recorded. A library that changed without anybody changing
  it is the thing that would otherwise be discovered much later.
...
The library is **reproducible**: the same source and the same flags produce a
byte-identical `libopus.a`, verified on every push by rebuilding a target on a second runner
and requiring the two to hash the same. Each archive records its own `sha256(library)` in
its MANIFEST, so that hash is comparable across runs, machines and releases — if it is the
...
checksum of one can only ever say *these are the bytes that were published* — useful for
catching a bad download, and nothing more. Three questions, and only one of them needs a
checksum this repository maintains:

| question | answered by | checked by |
|---|---|---|
| is the source the pinned upstream release? | `OPUS_SHA256` in `opus.env` | `build.sh`, before anything is unpacked |
| did the download arrive intact? | `SHA256SUMS`, a release asset | `build.rs` and `sync-prebuilt.sh --fetch` |
| is the archive on disk still what was built? | `sha256(library)` in each MANIFEST | `build.rs`, on every resolution path |
| is this the same library as last time? | the same `sha256(library)` | CI's `reproducible` job, across two runners |

**Only the first is a supply-chain check.** `opus.env` pins a third party's tarball, which
is the one link in the chain this repository does not control, and it is the one value kept
by hand. The other two are corruption checks: `SHA256SUMS` is published on the same release
as the archives it covers and the MANIFEST rides inside the archive it describes, so
whoever could replace one could replace both. They are worth having because corruption is
what actually happens — a truncated download, a cache half-written by a killed build, an
`.a` overwritten by hand — and each of those otherwise surfaces as a page of undefined
symbols rather than one sentence naming the file.

Neither is committed. `SHA256SUMS` is generated by the release job from the archives it
just built, which is what a checked-in file of our own hashes could never be: correct
without anybody remembering to regenerate it. And both are plain GETs from the URL
`build.rs` already uses — GitHub does publish a per-asset digest, but only through
`api.github.com`, which is a second request, rate-limited to 60/hour unauthenticated, and
JSON for a build script that has no dependencies to parse it with.

To check a download by hand:

```sh
curl -sSLO https://github.com/andrewtheguy/libopus-prebuilt/releases/latest/download/SHA256SUMS
...
answer that centrally; it was removed, because its first implementation generated its input
with `f64::sin` — which on Linux is glibc's, IFUNC-dispatched per CPU — and so reported
differences that were the measurement moving rather than the library. The archives were
